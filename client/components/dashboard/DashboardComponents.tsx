...
export function RouteTracker({ progress }: { progress: number }) {
  ...
  const vehicleX = vehiclePosition * 12.02;
  return <section className="route-tracker"><div className="route-visual"><div className="route-segment-assets" aria-hidden="true">{ROUTE_LINE_IMAGES.map((src, index) => <img key={src} className={`route-segment-image route-segment-image-${index + 1}`} src={src} alt="" />)}</div><div className="route-chevron-assets route-chevron-assets-behind" aria-hidden="true">{ROUTE_CHEVRON_IMAGES.map((src, index) => <img key={`behind-${src}`} className={`route-chevron-image route-chevron-image-${index + 1}`} src={src} alt="" />)}</div><div className="route-chevron-assets route-chevron-assets-ahead" aria-hidden="true">{ROUTE_CHEVRON_IMAGES.slice(1).map((src, index) => <img key={`ahead-${src}`} className={`route-chevron-image route-chevron-image-ahead-${index + 1}`} src={src} alt="" />)}</div><svg className="route-energy-svg" viewBox="0 0 1202 194" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="route-energy-gradient" x1="0" x2="1"><stop offset="0" stopColor="#00BFFF" /><stop offset="1" stopColor="#00FFFF" /></linearGradient><filter id="route-energy-glow" x="-30%" y="-300%" width="160%" height="700%"><feGaussianBlur stdDeviation="7" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter><clipPath id="route-progress-clip"><rect x="0" y="0" width={138 + 927 * boundedProgress / 100} height="194" /></clipPath></defs><path className="route-energy-remaining" d="M138 149.5 H1065" pathLength="100" style={{ strokeDasharray: `${100 - boundedProgress} ${boundedProgress}` }} /><path className="route-energy-completed" d="M138 149.5 H1065" pathLength="100" style={{ strokeDasharray: `${boundedProgress} ${100 - boundedProgress}` }} /><path className="route-energy-flow route-energy-flow-behind" d="M138 149.5 H1065" clipPath="url(#route-progress-clip)" style={{ opacity: 0.35 + boundedProgress / 150 }} /><path className="route-energy-pulse" d="M138 149.5 H1065" clipPath="url(#route-progress-clip)" /><ellipse className="route-energy-shadow" cx="569" cy="158" rx="119" ry="6.5" /><g className="route-energy-chevron-group route-energy-chevron-behind" transform={`translate(${vehicleX - 590} 0)`}><path d="M430 136 l23 13 -23 13 M453 136 l23 13 -23 13" /></g><g className="route-energy-chevron-group route-energy-chevron-ahead" transform={`translate(${vehicleX - 590} 0)`}><path d="M727 136 l23 13 -23 13 M750 136 l23 13 -23 13" /></g></svg><div className="route-flag route-start"><img src={ROUTE_START_FLAG_IMAGE} alt="Start" /></div><div className="route-flag route-end"><img src={ROUTE_END_FLAG_IMAGE} alt="Destination" /></div><div className="vehicle-container" style={{ left: `${vehiclePosition}%` }}><img className="glow-platform-asset" src={ROUTE_PLATFORM_IMAGE} alt="" /><span className="vehicle-shadow" /><svg className="glow-platform glow-platform-fallback" viewBox="0 0 393 60" aria-hidden="true"><defs><linearGradient id="vehicle-platform-gradient" x1="0" y1="0" x2="393" y2="0"><stop offset="0%" stopColor="#006C90" /><stop offset="50%" stopColor="#00BFFF" /><stop offset="100%" stopColor="#00E5FF" /></linearGradient><filter id="vehicle-platform-glow"><feGaussianBlur stdDeviation="8" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter></defs><path className="vehicle-platform-shape" d="M15 30 L0 15 H90 L110 8 H280 L305 15 H378 L393 30 L378 45 H305 L280 52 H110 L90 45 H0 Z" fill="url(#vehicle-platform-gradient)" filter="url(#vehicle-platform-glow)" /></svg><img className="vehicle-svg" src={ROUTE_SUV_IMAGE} alt="Vehicle" /></div></div></section>;
}
